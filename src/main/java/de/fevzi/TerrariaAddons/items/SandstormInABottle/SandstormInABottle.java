package de.fevzi.TerrariaAddons.items.SandstormInABottle;
...
/**
 * System that handles the Sandstorm in a Bottle accessory effect.
 * Provides a powerful double-jump ability with a sustained forward boost
 * when the player crouches while airborne. Consumes fuel during use and
 * refuels when the player lands. Creates sand particle effects during flight.
 */
public class SandstormInABottle extends EntityTickingSystem<EntityStore> {

    private final Map<PlayerRef, Integer> fuelRemaining = new ConcurrentHashMap<>();
    private static final String ITEM_ID = "SandstormInABottle";
    private static final double LEAP_FORCE = 8.0D;
    private static final double FORWARD_BOOST = 15.0D;
    ...
    public void tick(float delta,
        ...
        boolean hasBlizzard = AccessoryPouchSharedContainer.hasItemInPouch(player.getInventory(), playerRef.getUuid(), "BlizzardInABottle");
        ...
        boolean canUse = BoostOrderManager.canUseSandstorm(playerRef, hasCloud, hasBlizzard, hasItem);
        boolean isActive = BoostOrderManager.isActive(playerRef, BoostOrderManager.ActiveBoost.SANDSTORM);
        ...
        if (shouldActivate && holdingCrouch && !states.onGround && currentFuel >= FUEL_CONSUMPTION && canUse) {
            ...
            BoostOrderManager.setSandstormActive(playerRef);
        } else {
            BoostOrderManager.clearActiveBoost(playerRef, BoostOrderManager.ActiveBoost.SANDSTORM);
        }
    }
}